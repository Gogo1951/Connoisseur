...
| `UNIT_AURA` | On `player` and `pet`, only while buff food, scrolls or pet buff food is on and its group mode is active (`ns.UpdateAuraTracking`, which re-registers only when that wanted state flips, since every bag scan passes through it) |
...
- `UNIT_AURA` goes to `ns.HandleUnitAura` (`Features/Scanner-Character.lua`), which requests only when the Well Fed state changes or, for an enabled scroll type, the scroll buff's expiration or the largest conflicting class buff moves. Pet auras request whenever pet buff food is on; they change rarely.
...
The diffs run only out of combat. Below the lockdown guard these firehoses skip their diff and simply mark the update pending, so one unrelated aura or quest tick mid-fight still costs one rebuild when combat ends.
...
**Options panels.** `ns.WarmItemCache` (`Options/Options-Utilities.lua`) requests item data for the ids a panel is showing and repaints it through `NotifyChange` as answers land, polling every half second up to ten times, so an item row never sits on a bare id. There is one chain per registry name: a call while it runs (the repaint re-entering with still-cold ids, or a row added mid-load) merges its ids into that chain's pending set, and a genuinely new id restarts the attempt budget.
...
|---|---|---|
| `Engine.lua`, `BuildStandardBody` (ranked types only; a single-item body is never trimmed) | Stacked Healthstone lines first, then ranked fallback lines, bottom-up | The rank-1 `/use` line |
...
This has broken Era three times. Warlock Healthstones and Soulstones are distinctly named spells on Era, where they must be cast bare: appending `(Rank N)` builds a spell name that does not exist, and the `/cast` silently no-ops. On TBC they are one spell with numeric ranks, and the rank must be pinned. The split is declared as data, `rankIsTBCOnly` on those `ns.CONJURE_SPELLS` lists, and applied only by `ns.GetSmartSpell`, which leaves a flagged list's ranks bare on Era and Forever alone. Mage Conjure Food and Water are numeric-rank on both flavors, so their suffix is correct everywhere.
...
WoW Forever runs Classic content on the Retail client, so its API is Retail's, and it reports `WOW_PROJECT_ID` as `WOW_PROJECT_MAINLINE`. `ns.IS_FOREVER` names it, and because it runs Era's data it counts as Era everywhere the code branches on flavor: every such test reads `(ns.IS_ERA or ns.IS_FOREVER)`, `ns.CURRENT_EXPANSION` resolves to Classic, the warlock stone casts stay bare, and the Readiness Flask line stays hidden. Testing `ns.IS_ERA` alone is the trap: Forever then reads as a newer expansion, and it would offer TBC and Wrath items, buy no poison reagents, and pin ranks onto the warlock stones. The one deliberate exception is the Blinding Powder row repair, which stays Era-only because only Era ever shipped the bad ladder.

- **Legacy readers go through shims.** Forever has no `GetQuestLogTitle`, `GetSkillLineInfo`, `GetPetFoodTypes` or `GetMerchantItemInfo`. `Features/Utilities.lua` picks the namespaced replacement or the legacy global by existence, and each call site sees one shape on every client. Spell names come from `C_Spell.GetSpellName` everywhere, because Forever has no `GetSpellInfo`. The offline suites stub globals, so only running Diagnostics' API Endpoints on Forever catches a new legacy call.
- **The bank is purchased tabs.** Forever has no `BANK_CONTAINER` or bank bags. `ns.LoadRestockBankBags` reads the character bank's `C_Bank` tabs instead, and the bank open handler re-reads them because a tab can be bought at any visit.
- **No talent lines.** Forever's talents are Retail's, with no `GetNumTalentTabs` or `UnitCharacterPoints`, so the Readiness Report's spec and unspent-talent lines never show there, and the Spec switch hides wherever `GetNumTalentTabs` is missing.
- **Reads can be secret.** Forever returns secret values while the client restricts aura or unit data, and comparing one errors. The dispatcher holds aura scans until combat drops, but restriction is the client's call rather than combat's, so the aura entry points ask `C_Secrets.ShouldAurasBeSecret()` first: the player snapshot and the pet probe answer with their last reading while it is true, since a reset reading would look like "no buffs" and rebuild the macros around a buff the player still has, and `ns.ReportReadiness`, the one aura read that runs ahead of the combat guard, stays silent. The Readiness Report's own aura walks (soulstone, expiring buffs, flask) ask nothing themselves and rely on that gate. The target signature asks `C_Secrets.CanCompareUnitTokens` before `UnitIsUnit`, and the `UNIT_SPELLCAST_SUCCEEDED` handler skips a secret spell ID.
...
The bank's containers are rebuilt by `ns.LoadRestockBankBags` at every bank open: the fixed `BANK_CONTAINER` plus bank bags on Era and TBC, the purchased `C_Bank` tabs on Forever. A Shift held while opening the bank skips the run entirely, and since that path returns before the bank counts as open, it also skips the bank-close reminder. The debug trace (`ns.RestockerDebug`) is gated on the runtime-only diagnostics flag, so nothing can leave it on across sessions.
...
- **Buying.** `BuyMerchantItem` sells at most one stack per call, so buys are chunked, and "filled" is decided from units ordered, never from bag counts, which lag. `MERCHANT_SHOW` can double-fire, so a restock runs at most once a second, and a Shift held while opening the merchant skips buying for that visit. A run with no free bag slot stops before buying anything and says so.
...
- **Entering town.** Keyed off the client's own resting flag, which is on in inns and cities and nowhere else, so no zone list is maintained. Only a not-resting to resting edge counts, and three events reach the check because no one signal covers every arrival: `PLAYER_UPDATE_RESTING` (walking in), `PLAYER_ENTERING_WORLD` (login, hearth, portal, leaving an instance, where the flag crosses a loading screen with no update event behind it) and `PLAYER_CONTROL_GAINED` (landing off a flight path). A taxi counts as not in town for its whole duration, which leaves a real edge for the landing to trip. The check runs 2 seconds after the arrival event (5 after a fresh login), and nothing reminds until the first `PLAYER_ENTERING_WORLD` arms it. A `/reload` only records the current resting state, and a one-minute cooldown collapses clustered arrivals into one nudge. The chat line and the alert sound are separate settings and the code honors each alone, but the panel shows the sound as a sub-option of the chat line and hides it while the chat line is off.
...
- **Validate Data** runs `ns.DIAGNOSTIC_DATA_SOURCES`, one entry per data file, holding references to the `Data/` tables taken at load; that is why `Diagnostics.lua` loads after every data file, and a file left off the manifest is never checked. It proves an ID exists on this client, never that it is the item its row names: a wrong ID that happens to be a real item passes. A run flags an item NOT ON CLIENT at once when `C_Item.DoesItemExistByID` says it does not exist; the rest it requests in batches of 100 and polls, flagging one only after a long stretch with no progress, so a slow load is never cut off.
...
| Suite | Kind | Pins |
|---|---|---|
| `Readiness-Report-Test.lua` | Real files | Silence, the arena rule, the class and group gates, Expiring Soon, spec and unspent talents (Forever's missing APIs included), print order |
...
Retired keys are cleared explicitly at login, after the migration chain: every name in `ns.RETIRED_READY_CHECK_KEYS` on `global` (the old `readyCheck*` names plus `readinessReport`, the first master switch, which only looks like a live `readiness*` key), and the Restock List's `debugMessages` and `adoptedLegacyData`. A retired name is never reused. AceDB strips a value equal to its default at logout, but a value the player set stays in the saved file, and once its key leaves the defaults table nothing fills or strips it any more, so a reused name would read back a choice made under its old meaning.
...
1. `ns.AdoptStandaloneRestockerDB` (remove after 2026-09-29): moves the Restock List from `ConnoisseurRestockerDB` onto `ns.db.global.restocker`, only when the new table holds no saved item yet, then nils the old table so it can never run twice on the same data. It copies the old key names as they are, and step 2 converts them in the same login.
