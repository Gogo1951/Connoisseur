...
--[[
    ...
    sidebar takes its share of the panel first.

    There is no drop target. The game closes the bags and the bank when the
    options interface opens, so there is no way to have an item on the cursor
    and this panel in front of you at the same time; typing or shift-clicking an
    id into the add box is the only path that can actually work.
]]
...
--[[
    ...
    already covers everyone it just left.
]]
...
function ns.BuildIgnoreListOptions()
	--[[
	    The description lives on the root group, not in the scope panes: a
	    tree group renders its own non-group args once, full width, between
	    the panel title and the tree, so the copy reads once instead of
	    repeating in every pane.
	]]
	local args = {
		descIntro = ns.OptionsDesc(L["OPTIONS_IGNORE_LIST_DESCRIPTION"], 1),
